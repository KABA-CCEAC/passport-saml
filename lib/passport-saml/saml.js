...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(function() {
    if (self.options.chain && xml.indexOf('X509Certificate>') > 0) {
      var certInResponse = xml.substring(xml.indexOf('X509Certificate>') + 16);
      certInResponse = certInResponse.substring(0, certInResponse.indexOf('X509Certificate>') - 5);
      certInResponse = '-----BEGIN CERTIFICATE-----\n' + certInResponse;
      certInResponse += '\n-----END CERTIFICATE-----';

      return Q.ninvoke(pem, 'verifySigningChain', certInResponse, self.options.chain)
        .then(function(result) {
          if (!result)
